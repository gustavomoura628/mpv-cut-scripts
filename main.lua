...
ACTIONS.ENCODE_WHATSAPP = function(d)
	local args = {
		"ffmpeg",
		"-nostdin", "-y",
		"-loglevel", "error",
		...
		"-preset", "superfast",
		"-c:s", "mov_text",
		"-vf", "subtitles='"..d.infile.."'",
		"-vcodec", "libx264",
		"-acodec", "aac",
		"-report",
		utils.join_path(d.indir, "ENCODE_WHATSAPP_" .. d.channel .. "_" .. d.infile_noext .. "_FROM_" .. d.start_time_hms .. "_TO_" .. d.end_time_hms .. ".mp4")
	}
	mp.command_native_async({
		name = "subprocess",
		args = args,
...
ACTIONS.ENCODE_WHATSAPP_CLAUDE = function(d)
    local function run_ffmpeg(args, callback)
        mp.command_native_async({
            name = "subprocess",
            args = args,
            playback_only = false,
    ...
    local args1 = {
        ...
        "-preset", "superfast",
		"-c:s", "copy",
        utils.join_path(d.indir, "temp_output.mkv")
    }
    ...
    local args2 = {
        ...
        "-nostdin", "-y",
        "-loglevel", "info",
        "-i", utils.join_path(d.indir, "temp_output.mkv"),
        -- Add your second ffmpeg command arguments here
        "-vf", "subtitles='".."temp_output.mkv".."'",
        "-vcodec", "libx264",
        "-acodec", "aac",
		------------
        utils.join_path(d.indir, "ENCODE_WHATSAPP_" .. d.channel .. "_" .. d.infile_noext .. "_FROM_" .. d.start_time_hms .. "_TO_" .. d.end_time_hms .. ".mp4")
    ...
    run_ffmpeg(args1, function()
        print("First ffmpeg call completed")
        run_ffmpeg(args2, function()
            print("Second ffmpeg call completed")
            os.remove(utils.join_path(d.indir, "temp_output.mkv"))
            print("Temporary file removed")
        end)
    end)
end

ACTIONS.ENCODE_WHATSAPP_CLAUDE_IMAGE_SUBS = function(d)
    local function run_ffmpeg(args, callback)
        mp.command_native_async({
            name = "subprocess",
            args = args,
    ...
    local args2 = {
		...
		"-map", "0:a",
		"-c:a", "copy",
        utils.join_path(d.indir, "temp_output_2.mkv")
		------------
    }
    ...
    local args3 = {
        ...
        "-nostdin", "-y",
        "-loglevel", "info",
        "-i", utils.join_path(d.indir, "temp_output_2.mkv"),
        -- Add your second ffmpeg command arguments here
        "-vcodec", "libx264",
        "-acodec", "aac",
		------------
    ...
    run_ffmpeg(args1, function()
        ...
        run_ffmpeg(args2, function()
            print("Second ffmpeg call completed")
			run_ffmpeg(args3, function()
				print("Third ffmpeg call completed")
				os.remove(utils.join_path(d.indir, "temp_output.mkv"))
				os.remove(utils.join_path(d.indir, "temp_output_2.mkv"))
